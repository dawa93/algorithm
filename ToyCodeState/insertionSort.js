...
const insertionSort = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    let idx = i;
    while (arr[idx] > arr[idx + 1] && arr[idx + 1] !== undefined) {
      let temp = arr[idx + 1];
      arr[idx + 1] = arr[idx];
      arr[idx] = temp;
      idx--;
...
//한줄글 : advanced 는 아직 잘 모르겠당.... 지금 작성한 코드도 사실 완벽한 내코드는 아니다. insertionSort가 무엇인지 알아보다가 어떠분의 코드를 보고 이해하고 적어봤다. 알고리즘 너무 어렵다....
