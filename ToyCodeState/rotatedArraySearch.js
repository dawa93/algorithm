...
const rotatedArraySearch = function (rotated, target) {
  ...
  while (left <= right) {
    ...
    if (rotated[left] <= target && rotated[mid - 1] >= target) {
      right = mid - 1;
    } else if (rotated[mid + 1] <= target && target <= rotated[right]) {
      left = mid + 1;
    } else {
      return -1;
    }
...
//한줄글 : binary search에 살짝 익숙해진거같다. 이전에 레퍼런스를 보고 공부해보니 어느정도 어떻게 하면 되겠다는것을 알게되었다. 그래도 마냥 쉽지만은 않았다. 그래도 실력이 조금씩 는거 같아서 좋다 ~
